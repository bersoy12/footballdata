...
def gap_report_endpoint(
    tournament_id: int,
    season_id: int,
    total_rounds: Optional[int] = Query(default=None, description="Sezondaki toplam hafta sayısı (bilinmiyorsa boş bırakın)")
):
    """
    Verilen turnuva ve sezon için veritabanındaki eksiklikleri raporlar.
    - Eksik round'lar (maç hiç çekilmemiş)
...
def gap_report(
    ...
    # Her kolonu ayrı ayrı çek
    all_match_ids      = fetch_data("match_id",      "match")
    all_tournament_ids = fetch_data("tournament_id", "match")
    all_season_ids     = fetch_data("season_id",     "match")
    all_rounds         = fetch_data("round",         "match")

    # Zip'le birleştir
    all_matches = [
        {"match_id": mid, "tournament_id": tid, "season_id": sid, "round": r}
        for mid, tid, sid, r in zip(all_match_ids, all_tournament_ids, all_season_ids, all_rounds)
    ]
    ...
    fetched_rounds = sorted({int(m["round"]) for m in season_matches if m.get("round") is not None})
    last_round = max(fetched_rounds) if fetched_rounds else None

    # Eksik round'lar
    if total_rounds:
        all_rounds = set(range(1, total_rounds + 1))
        missing_rounds = sorted(all_rounds - set(fetched_rounds))
    else:
        # Çekilen round'lar arasındaki boşlukları bul
        if fetched_rounds:
    ...
    # 2. Alt tablo eksikliklerini bul
    def missing_in_table(table_name: str) -> list:
        existing = fetch_data("match_id", table_name)
        existing_ids = set()
    ...
    missing_statistic = missing_in_table("statistic")
    missing_incident  = missing_in_table("incident")
    missing_momentum  = missing_in_table("momentum")
    ...
    return {
        ...
        "matches_without_statistic": missing_statistic,
        "matches_without_incident": missing_incident,
        "matches_without_momentum": missing_momentum,
    }